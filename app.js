...
// 검색 결과 드롭다운을 보여줄 때, 화면 하단 공간이 부족하면 입력창 위쪽으로 펼친다.
function positionDropdown(inputEl, dropdownEl) {
  const rect = inputEl.getBoundingClientRect();
  const spaceBelow = window.innerHeight - rect.bottom;
  const dropdownHeight = Math.min(dropdownEl.scrollHeight || 320, 340);
  const openUpward = spaceBelow < dropdownHeight + 16 && rect.top > dropdownHeight;

  dropdownEl.classList.toggle('drop-up', openUpward);
}
